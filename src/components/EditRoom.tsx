...
import { UpdateRoom } from "@/app/api/Action";
...
const EditRoom = ({ EditRoomDetials }: EditRoomDetialsProps) => {
  const [amenit, setAmenities] = React.useState<Selection>(new Set([]));
  const [loading, setLoading] = useState(false);
  const [image, setImage] = useState({
    preview: EditRoomDetials.image,
    file: "",
  });
  ...
  return (
    <div className="w-full my-10">
      <Card className="flex w-full text-default-800 md:w-[50%] mx-auto p-5 flex-col">
        <h1 className="text-2xl text-center font-bold mt-2">Edit Room</h1>
        <p className="mt-1 text-center mb-5">
          Kindly fill in the correct room details
        </p>
        ...
        <form onSubmit={handleSubmit(submit)}>
          <Card className="flex flex-col gap-3 p-5">
            ...
            <Input
              ...
              defaultValue={EditRoomDetials.name}
            />
            <Input
              className="w-full"
              label={"Room Size"}
              ...
              defaultValue={EditRoomDetials.room_size}
            />
            <Input
              className="w-full"
              ...
              defaultValue={EditRoomDetials.price_per_night.toString()}
            />
            <Input
              className="w-full"
              ...
              endContent={
                <MdSensorOccupied size={20} className="text-default-800" />
              }
              color="default"
              {...register("maxOccupancy")}
              isInvalid={!!errors.maxOccupancy}
              errorMessage={errors.maxOccupancy?.message}
              defaultValue={EditRoomDetials.max_occupancy.toString()}
            />
                ...
                render={({ field: { onChange, value } }) => {
                  return (
                    <Select
                      ...
                      selectedKeys={[EditRoomDetials.bed_type]}
                    >
            ...
            <div className="flex w-full flex-col gap-2">
              <Select
                ...
                selectedKeys={
                  Array.from(amenit).length > 0
                    ? amenit
                    : EditRoomDetials.amenities
                }
                variant="bordered"
            ...
            <Textarea
              ...
              defaultValue={EditRoomDetials.description}
            />
            {loading ? (
              <>
                <Button
